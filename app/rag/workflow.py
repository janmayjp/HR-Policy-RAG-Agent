...
def llm():
    ...
    if _llm is None:
        ...
        if not api_key:
            raise RuntimeError("OPENROUTER_API_KEY is missing in settings / .env")

        base_url = getattr(settings, "openai_api_base", "https://openrouter.ai/api/v1")

        _llm = ChatOpenAI(
            model=settings.openai_model, # Resolves to "dots-studio/dots-3-note-preview:free"
            temperature=0,
            api_key=api_key,
            base_url=base_url,
            default_headers={
                "HTTP-Referer": "http://localhost:8080", # Required by OpenRouter for rankings/tracking
                "X-Title": "HR-Policy-RAG-Copilot",
            }
        )
    return _llm
...
"""
    try:
        ...
        route_val = parsed.get("route", "kb")
    except Exception as e:
        logger.warning(f"Routing parse failed ({e}), defaulting to 'kb'")
        route_val = "kb"

    return {
        "source_used": route_val,
        "trace": add_trace(state, f"Router → {route_val.upper()}"),
    }
...
def grade_kb(state: AgentState):
    parser = JsonOutputParser(pydantic_object=EvidenceGrade)
    context = "\n\n".join(
...
"""
    try:
        response = llm().invoke(prompt)
        parsed = parser.parse(response.content)
        grade_val = parsed.get("grade", "weak")
    except Exception as e:
        logger.warning(f"KB Grading parse failed ({e}), defaulting to 'weak'")
        grade_val = "weak"

    return {
        "kb_grade": grade_val,
        "trace": add_trace(state, f"KB evidence grade → {grade_val.upper()}"),
    }
...
"""
    try:
        ...
        logger.warning(f"Web Grading parse failed ({e}), defaulting to 'weak'")
        grade_val = "weak"

    return {
        "web_grade": grade_val,
        "trace": add_trace(state, f"Web evidence grade → {grade_val.upper()}"),
    }
